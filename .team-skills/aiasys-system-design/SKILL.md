...
AIASys 是**本地桌面应用**，不是云服务。每个用户在自己的机器上运行自己的实例。这对 Bug 判断和修复设计有直接影响：

- **文件大小限制不是 Bug**：用户操作自己机器上的文件，Agent 读写、知识库上传、工作区导入等操作不应加硬性大小限制。如果发现缺少大小限制，不要当作 Bug 修复
- **速率限制不是 Bug**：没有外部攻击者，不需要对用户自己的 API 调用加速率限制
- **请求大小限制不是 Bug**：用户上传自己的文件到自己的知识库，不应限制文件大小
- **多租户隔离不是 Bug**：每个用户独立进程，不需要用户间资源隔离
- **真正的 Bug 是**：崩溃、死锁、数据损坏、功能不可用、内存泄漏、非原子写入导致的数据丢失

Agent 在评估安全/资源类问题时，必须先确认：这个问题在本地桌面应用场景下是否真的构成威胁？如果是云服务思维带入的误判，应明确标注为"非 Bug（本地部署不适用）"。
