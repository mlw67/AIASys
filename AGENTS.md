...
AIASys 是**本地部署的桌面应用**，每个用户在自己的机器上运行自己的实例，不是云服务或 SaaS 平台。这一约束决定了以下设计边界：

- **不做多租户隔离**：每个用户运行自己的进程，数据存储在自己的文件系统上，不需要用户间资源隔离
- **不做文件大小限制**：用户操作的是自己机器上的文件，Agent 读写文件、知识库上传、工作区导入等操作不应加硬性大小限制。文件大小是用户自己的事，不是系统安全边界
- **不做外部 DoS 防护**：没有外部攻击者向 API 发送恶意请求的场景，不需要对用户自己的操作加速率限制、请求大小限制等
- **安全关注点是意外损坏**：安全机制的关注点是防止 Agent 误操作或 prompt injection 导致的意外文件损坏、凭证泄露，而非防止恶意外部攻击
- **资源限制是用户偏好**：超时、并发数、内存上限等是用户体验优化，不是安全硬性要求。这些限制应可配置，不应硬编码为不可绕过的校验

Agent 在评估 Bug 和设计修复方案时，必须区分：
- **真正的 Bug**：会导致崩溃、死锁、数据损坏、功能不可用的代码缺陷
- **云服务思维**：在本地部署场景下不需要的多租户隔离、文件大小限制、DoS 防护、速率限制等
...
AIASys 桌面端提供跨平台绿色版安装辅助脚本，让 AI 和人类都能安全、可预期地安装/升级。

- **脚本位置**：`apps/desktop/scripts/install.cjs`
- **支持平台**：Windows、macOS、Linux
- **安装包形态**：绿色版 zip（已替代 NSIS 作为自动化/AI 安装首选）

**为什么优先用绿色版 zip + 安装脚本？**

NSIS 安装程序在自动化环境中可能出现静默安装无响应、UAC 弹窗阻塞、路径解析异常等问题。绿色版 zip 配合 Node.js 安装脚本可以：

- 自动检测运行中的 AIASys 进程并安全终止（按 PID，不按进程名全局杀）
- 自动备份旧版本
- 解压到固定/指定目录
- 创建系统快捷方式 / `.desktop` 入口
- 验证关键文件存在
...
```bash
# Windows
node apps/desktop/scripts/install.cjs dist/AIASys-0.4.25-win.zip

# macOS
node apps/desktop/scripts/install.cjs dist/AIASys-0.4.25-mac.zip

# Linux
node apps/desktop/scripts/install.cjs dist/AIASys-0.4.25-linux.zip
```

指定目标目录：

```bash
node apps/desktop/scripts/install.cjs dist/AIASys-0.4.25-win.zip "C:\tools\AIASys"
```

**AI 安装推荐流程**：

1. 构建/下载对应平台的绿色版 zip
2. 运行 `node apps/desktop/scripts/install.cjs <zip 路径>`
3. 读取脚本输出的 `[INFO]` 日志确认安装成功
4. 如需启动验证，直接运行安装目录下的可执行文件：
   - Windows：`%LOCALAPPDATA%\Programs\AIASys\AIASys.exe`
   - macOS：`open /Applications/AIASys.app`
   - Linux：`~/.local/share/AIASys/AIASys --no-sandbox`

**构建绿色版 zip**：

`package.json` 中三端均已配置 `zip` target：

- Windows：`npx electron-builder --win zip`
- macOS：`npx electron-builder --mac zip`
- Linux：`npx electron-builder --linux zip`

CI workflow 也应同步产出 zip 包，供安装脚本消费。
...
- AI 在 Windows 侧执行安装包测试、后台启动、注册表修改等需要管理员权限的操作时，应主动使用已提供的管理员凭据（锁屏密码 `xiaoke`）完成自动化，而不是等待用户手动点击 UAC/确认框。
- AIASys 安装包应通过 NSIS 脚本自动启用 Windows 长路径支持（`LongPathsEnabled`），并在需要时自动申请管理员权限；Agent 不应依赖用户手动处理路径长度或 UAC 弹窗。
- 为便于 Agent 在无显示/远程桌面环境下调试，Windows 桌面端打包时应预设以下非交互入口：
  - **首选绿色版 zip + `apps/desktop/scripts/install.cjs`** 安装脚本，避免 NSIS 静默安装无响应或 UAC 阻塞。Agent 应优先用脚本安装，而不是 `/S` 静默安装。
  - 保留 NSIS 安装程序作为人类手动安装入口，支持 `/S` 静默安装，且安装程序自身能自动提升管理员权限。
  - 支持通过环境变量指定后端/前端端口（`AIASYS_DESKTOP_BACKEND_PORT`、`AIASYS_DESKTOP_FRONTEND_PORT`），避免多实例端口冲突。
  - 支持 `AIASYS_DESKTOP_DISABLE_GPU=1` 及 `--disable-namespace-sandbox`/`--disable-setuid-sandbox` 等启动参数，便于无头或 GPU 受限环境调试验证。
- 当前阶段若遇到无法自动完成的 UAC/确认框，Agent 应显式告知用户哪一步被阻塞、需要点击什么，而不是默默卡住。
