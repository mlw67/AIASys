...
from app.services.claw.adapters.base import (  # noqa: E402
...
from app.services.claw.adapters.constants import get_hermes_home  # noqa: E402
from app.services.claw.adapters.models import Platform, PlatformConfig  # noqa: E402
from app.services.claw.adapters.status_shim import (  # noqa: E402
    acquire_scoped_lock,
    release_scoped_lock,
)
...
def _strip_markdown_to_plain_text(text: str) -> str:
    ...
    horizontal rules, \\r\\n normalisation).
    """
    from app.services.claw.adapters.helpers import strip_markdown
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        if self._connection_mode not in {"websocket", "webhook"}:
            logger.error(
                "[Feishu] Unsupported FEISHU_CONNECTION_MODE=%s. Supported modes: websocket, webhook.",  # noqa: E501
    ...
    async def edit_message(
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "[Feishu] Invalid post update payload rejected by API; falling back to plain text"  # noqa: E501
                )
    ...
    def _media_batch_key(self, event: MessageEvent) -> str:
        from app.services.claw.adapters.session_shim import build_session_key
